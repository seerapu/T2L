...
"""
        ...
        # Deploy to production with debugging
        try:
            if branch_name.lower() in ['main', 'master', 'production']:
                # Debug information for production deployment
                st.info("🔍 DEBUG: Starting production deployment...")
                st.info(f"🔍 DEBUG: Project ID: {project_name}")
                st.info(f"🔍 DEBUG: Attempting deploy_to_production...")
                
                try:
                    result = sdk.deploy_to_production(project_id=project_name)
                    st.info(f"🔍 DEBUG: Deploy result: {result}")
                    st.success(f"🚀 Successfully deployed '{project_name}' to production!")
                    return True
                except Exception as deploy_error:
                    st.error(f"🔍 DEBUG: deploy_to_production failed: {deploy_error}")
                    st.info(f"🔍 DEBUG: Error type: {type(deploy_error).__name__}")
                    
                    # Try alternative deployment method
                    st.info("🔍 DEBUG: Trying alternative deployment method...")
                    try:
                        # Get current production branch
                        git_branches = sdk.all_git_branches(project_id=project_name)
                        st.info(f"🔍 DEBUG: Available branches: {[b.name for b in git_branches]}")
                        ...
                        st.info(f"🔍 DEBUG: Current production branch: {production_branch}")
                        
                        if production_branch:
                            result = sdk.deploy_ref_to_production(
                                project_id=project_name, 
                                branch=production_branch
                            )
                            st.info(f"🔍 DEBUG: deploy_ref_to_production result: {result}")
                            st.success(f"🚀 Successfully deployed '{project_name}' from branch '{production_branch}'!")
                            return True
                        else:
                            st.error("🔍 DEBUG: No production branch found")
                            return False
                            
                    except Exception as alt_error:
                        st.error(f"🔍 DEBUG: Alternative deployment also failed: {alt_error}")
                        st.error(f"🔍 DEBUG: Alternative error type: {type(alt_error).__name__}")
                        return False
            else:
